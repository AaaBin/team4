...
class FlowerController extends Controller
{
    ...
    {
        $all_flower_datas = flower::all();
        return view('admin/flower/index', compact("all_flower_datas"));
    }
    ...
    public function sort_down(Request $request)
    {
        ...
        $item = flower::find($request->data_id);
        // 抓到主資料的目前sort值
        $sort_value = $item->sort;
        // 排序後以first抓到第一筆，存成變數(目標資料)
        $target = flower::where('sort','<',$sort_value)->orderby('sort','desc')->first();
    ...
    public function sort_up(Request $request)
    {
        ...
        // 抓到主資料的目前sort值
        $sort_value = $item->sort;
        $target = flower::where('sort','>',$sort_value)->orderby('sort','asc')->first();
